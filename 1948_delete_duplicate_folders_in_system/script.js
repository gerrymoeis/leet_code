...
function markDuplicates(trie, nodeIndex, seenHashes) {
    ...
    if (currentNode.children) {
      for (const [folderName, currentIndex] of Object.entries(currentNode.children)) {
        const childHash = markDuplicates(trie, currentIndex, seenHashes)
        const childWithName = folderName + ':' + childHash
        childrenHashes.push(toHash(childWithName))
      }
    }
    ...
    if (structureKey.length > 0) {
      ...
      if (seenHashes.has(structureHash)) {
        ...
        markSubtree(trie, originalNodeIndex)
        markSubtree(trie, nodeIndex)
      }
      else {
